...
func (m *UserSysModel) GetByEmail(email string) (*User, error) {
	user := User{Email: email}
	query := `
	Select Id, FullName, Hash, IsActive
	FROM Users 
	WHERE Email = ?`
	res := m.DB.QueryRow(query, email)
	err := res.Scan(&user.Id, &user.FullName, &user.passHash, &user.IsActive)
	if err != nil {
		return nil, err
	}
	return &user, nil
